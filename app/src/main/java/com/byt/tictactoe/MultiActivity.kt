...
import com.byt.tictactoe.databinding.ActivityMultiBinding
...
if(turn) {
    if (i.getTag() != R.drawable.x && i.getTag() != R.drawable.o) {
        ...
        cheked.remove(i)
        Mark(i.id, 1, cheked.size)


    }
}

                else if(cheked.size>0) {
                  if( i.getTag()!=R.drawable.x && i.getTag()!=R.drawable.o) {
                      if(!turn){
...
//                              Mark(cheked[temp]!!.id,2,i);
    ...
    public fun Mark (value:Any,type:Int,checkturn:Int){
...
//        Log.d("cheking","${display}")
    ...
    fun SetWinner(type:Int,goal:Any){
        ...
        if(type==1){
            binding.type.text="Winner"
            binding.winnerimage.setImageResource(R.drawable.x)
            binding.showwinner.updateLayoutParams{
                width=350
            ...
            binding.type.text="Winner"
            binding.winnerimage.setImageResource(R.drawable.o)
            binding.showwinner.updateLayoutParams{
                width=350
